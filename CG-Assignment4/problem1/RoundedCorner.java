...
public class RoundedCorner extends Frame {
    public static void main(String[] args) {
        if (args.length == 0)
            System.out.println("Use filename as program argument.");
        else
            new RoundedCorner(args[0]);
    }

    RoundedCorner(String fileName) {
...
class CvFractalGrammars extends Canvas {
    ...
    double xLast, yLast, dir, rotation, dirStart, fxStart, fyStart,
            lengthFract, reductFact, last_rad;
            ...
            switch (ch) {
                case 'F': // Step forward and draw
                    ...
                    if (depth == 0) {
                        ...
                        // This part of code is the main implementation of rounded corner
                        if (last_rad != rad) {
                            double d = (rad + last_rad) * 0.5;
                            drawTo(g, xLast + len * Math.cos(d) / 3, yLast + len * Math.sin(d) / 3);
                            last_rad = rad;
                        }
